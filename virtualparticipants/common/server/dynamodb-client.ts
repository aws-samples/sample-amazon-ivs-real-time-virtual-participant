...
export class DynamoDBVpClient {
  ...
  constructor() {
    ...
    // Stages table is optional - only needed if using stage lookup functionality
    if (this.stagesTableName) {
      console.info(
        `DynamoDB client configured with stages table: ${this.stagesTableName}`
      );
    }
  }
   ...
   * @throws Error if STAGES_TABLE_NAME is not configured
   */
  async getStageRecordByArn(stageArn: string): Promise<StageRecord | null> {
    if (!this.stagesTableName) {
      throw new Error(
        'STAGES_TABLE_NAME environment variable is required for stage lookup'
      );
    }
...
export { VpStatus };
export type { StageRecord, VpRecord };
