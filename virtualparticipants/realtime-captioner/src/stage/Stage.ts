...
} from '@utils/speech-recognition-integration';
import {
  Stage,
...
class CaptionerStage extends Stage {
  ...
  constructor(token: string, socket: WebSocket | null) {
    ...
    // Set up SEI caption sender for Speech Recognition integration
    this.setupSEICaptionSender();
    ...
    this.on(STAGE_PARTICIPANT_STREAMS_ADDED, (participantInfo, streams) => {
      ...
      if (!participantInfo.isLocal) {
        ...
        if (!audioStream) {
          console.warn(
            `[${userId}] No audio stream found for participant: ${participantInfo.id}`
          );
        ...
        console.info(
          `[${userId}] Found audio stream for participant ${participantInfo.id}: kind=${audioStream.mediaStreamTrack.kind}, enabled=${audioStream.mediaStreamTrack.enabled}, readyState=${audioStream.mediaStreamTrack.readyState}`
        );
        ...
        // Play audio and then capture the stream for Speech Recognition

        audioElement
          .play()
          .then(() => {
            // Capture the audio stream from the playing audio element
            ...
            if (capturedTrack) {
              console.info(
                `[${userId}] Captured audio track from audio element for participant ${participantInfo.id}: kind=${capturedTrack.kind}, enabled=${capturedTrack.enabled}, readyState=${capturedTrack.readyState}`
              );

              try {
                // Boost audio gain using Web Audio API before passing to Speech Recognition
                const audioContext = new AudioContext();
                const source = audioContext.createMediaStreamSource(
                  new MediaStream([capturedTrack])
                );
                const gainNode = audioContext.createGain();

                // Input audio may be too quiet - apply gain
                ...
                console.info(
                  `[${userId}] Applied ${gainNode.gain.value}x gain boost to audio for participant ${participantInfo.id}`
                );

                speechRecognitionIntegration.addAudioInput(
                  participantInfo.id,
                  boostedTrack
                );
                console.info(
                  `[${userId}] Successfully added boosted audio track to Speech Recognition for participant ${participantInfo.id}`
                );
              } catch (error) {
                console.error(
                  `[${userId}] Failed to add captured audio track to Speech Recognition for participant ${participantInfo.id}:`,
                  error
                );
              }
            } else {
              console.error(
                `[${userId}] No audio track captured from audio element for participant ${participantInfo.id}`
              );
            }
          ...
          .catch((error) => {
            console.warn(
              `[${userId}] Failed to play audio element for participant ${participantInfo.id}:`,
              error
            );
          });
      }
    ...
    this.on(STAGE_PARTICIPANT_STREAMS_REMOVED, (participantInfo, _streams) => {
      ...
      if (!participantInfo.isLocal) {
        ...
        // Remove audio input from Speech Recognition integration
        speechRecognitionIntegration.removeAudioInput(participantInfo.id);
   ...
   * Set up SEI caption sender for Speech Recognition integration
   */
  private setupSEICaptionSender(): void {
    const captionSender = async (message: CaptionSEIMessage): Promise<void> => {
      try {
        // Ensure we're connected and publishing before sending SEI
        if (!this.connected || !this.publishing) {
          console.warn(
            '[SEI Caption] Stage not connected or not publishing, skipping SEI send'
          );

          return;
        }

        // Send caption via SEI with repeat count for reliability
        const repeatCount = message.partial ? 3 : 8; // More repeats for final captions
        ...
        console.info(
          `[SEI Caption] Successfully sent ${message.partial ? 'partial' : 'final'} caption via SEI`
        );
      } catch (error) {
        console.error(
          '[SEI Caption] Failed to send caption via SEI:',
    ...
    // Register the caption sender with Speech Recognition integration
    speechRecognitionIntegration.setSEICaptionSender(captionSender);
    console.info(
      '[SEI Caption] SEI caption sender configured for Speech Recognition integration'
    );
  }
}

export default CaptionerStage;
