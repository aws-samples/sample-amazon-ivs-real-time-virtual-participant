/**
 * Realtime AI Output Audio Processor Worklet
 * Handles Realtime AI audio response playback with buffering
 * - Receives PCM16 audio chunks from Realtime AI
...
interface OutputProcessorMessage {
...
class OutputProcessor extends AudioWorkletProcessor {
  ...
  private sourceSampleRate = 24000; // Realtime AI outputs 24kHz
  ...
  constructor() {
    ...
    console.info(
      `Output processor: Initialized with worklet sample rate: ${this.targetSampleRate}Hz`
    );
    console.info(
      `Output processor: Source sample rate (Realtime AI): ${this.sourceSampleRate}Hz`
    );

    this.port.onmessage = (event: MessageEvent<OutputProcessorMessage>) => {
      ...
      switch (type) {
        ...
        case 'initial-buffer-length':
          if (event.data.bufferLength) {
            ...
            console.info(
              `Output processor: Changed initial buffer length to ${event.data.bufferLength}`
            );
          }
        ...
        case 'set-sample-rate':
          if (event.data.sampleRate) {
            ...
            console.info(
              `Output processor: Sample rate updated from ${oldRate}Hz to ${this.targetSampleRate}Hz`
            );
          }
        ...
        case 'barge-in':
          ...
          console.info('Output processor: Buffer cleared for barge-in');
          break;

        default:
          console.warn(
            `Output processor: Unknown message type: ${event.data.type}`
          );
          break;
      }
    };
  ...
  private processAudioData(audioBuffer: ArrayBuffer): void {
    try {
      ...
      console.error('Output processor: Error processing audio data:', error);
    }
  }
  ...
  /**
   * Set the target sample rate (called from main thread)
   */
  setTargetSampleRate(sampleRate: number): void {
    this.targetSampleRate = sampleRate;
    console.info(`Output processor: Set target sample rate to ${sampleRate}Hz`);
  }
