...
import { createVideo, deleteVideos } from '@utils/dealer.utils';
import { realtimeAIIntegration } from '@utils/realtime-ai-integration';
...
// Initialize Realtime AI integration
async function initializeRealtimeAI(
  audioElem: HTMLAudioElement
): Promise<void> {
  try {
    console.info(
      '[initializeRealtimeAI] Starting Realtime AI integration initialization'
    );

    await realtimeAIIntegration.initialize(audioElem);
    console.info(
      '[initializeRealtimeAI] Realtime AI integration initialized successfully'
    );
  } catch (error) {
    console.error(
      '[initializeRealtimeAI] Realtime AI integration not available:',
      error instanceof Error ? error.message : String(error)
    );
  }
}

function loadStageVideo(assetName: string) {
  const videoAssetUrl = assetName ? `/${assetName}` : '/video-1.mp4';

  console.info('Loading video asset: ', videoAssetUrl);

  if (!stage) {
    console.error('loadStageVideo: No Stage');
...
async function loadStageWithAudioVideo(): Promise<void> {
  if (!stage) {
  ...
  try {
    ...
    console.info(
      '[loadStageWithAudioVideo] Initializing Realtime AI integration'
    );

    // Initialize Realtime AI with the audio element and wait for completion
    await initializeRealtimeAI(audioParticipant.element);

    // Start the audio capture process immediately after Realtime AI initialization
    ...
    console.error('[loadStageWithAudioVideo] Error context:', {
      ...
      realtimeAIReady: realtimeAIIntegration.isAudioOutputReady(),
      realtimeAIConnected: realtimeAIIntegration.connected
    });
...
// Initialize stage with video and audio
async function initializeStage(vpInfo: VirtualParticipant): Promise<void> {
  ...
  try {
    ...
    if (vpInfo.assetName) {
      console.info('[initializeStage] Loading video asset');
      loadStageVideo(vpInfo.assetName);
    } else {
      console.info('[initializeStage] Setting up audio and video');
      await loadStageWithAudioVideo();
    }
...
function handleVpUpdate(update: VirtualParticipantSubscriptionData): void {
  ...
  if (vpInfo.status === VirtualParticipantStatus.INVITED) {
    ...
    deleteVideos();
    deleteAudioElems();
    realtimeAIIntegration.disconnect();
