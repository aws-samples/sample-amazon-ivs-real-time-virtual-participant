...
import { RealtimeAIMessage, RealtimeAIProxy } from './realtime-ai-proxy';
...
export class VirtualParticipantWebSocketServer {
  ...
  private realtimeAIProxy: RealtimeAIProxy;
  ...
  constructor(port = 3001) {
    ...
    // Initialize Realtime AI proxy
    this.realtimeAIProxy = new RealtimeAIProxy();
  ...
  private handleClientMessage(
    ...
    switch (message.type) {
      // Realtime AI-related messages
      case 'session.update':
        this.handleRealtimeAISessionCreate(client, message);
        break;
      ...
      case 'response.cancel':
        this.forwardToRealtimeAI(client, message as RealtimeAIMessage);
        break;
  ...
  private forwardToRealtimeAI(
    client: ClientConnection,
    message: RealtimeAIMessage
  ): void {
    const success = this.realtimeAIProxy.forwardMessage(client.id, message);

    if (!success) {
      this.sendError(
        client.ws,
        'No active Realtime AI session or message forwarding failed'
      );
    }
  }

  private handleRealtimeAISessionCreate(
    client: ClientConnection,
    sessionConfig: unknown
  ): void {
    this.realtimeAIProxy.createSession(
      client.id,
      sessionConfig,
      (message: RealtimeAIMessage) => {
        // Forward Realtime AI messages to client
  ...
  private sendMessage(ws: WebSocket, message: Record<string, unknown>): void {
    if (ws.readyState === WebSocket.OPEN) {
      if (message.type === 'response.output_audio.delta') {
        const truncatedMessage = { ...message, delta: 'TRUNCATED_AUDIO_DATA' };
        console.info(
  ...
  private cleanup(client: ClientConnection): void {
    ...
    this.realtimeAIProxy.closeSession(client.id);
  ...
  public close(): void {
    ...
    // Close Realtime AI proxy
    this.realtimeAIProxy.cleanup();
  ...
  public getServerStatus(): {
    clients: number;
    realtimeAIActive: boolean;
    appSyncActive: boolean;
  } {
    return {
      clients: this.clients.size,
      realtimeAIActive: true, // Realtime AI proxy is always available
