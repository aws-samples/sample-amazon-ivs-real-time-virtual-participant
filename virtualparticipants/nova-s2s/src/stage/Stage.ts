...
} from '@utils/nova-s2s-integration';
import {
  Stage,
...
class DealerStage extends Stage {
  ...
  constructor(token: string, socket: WebSocket | null) {
    ...
    // Set up SEI transcript sender for OpenAI integration
    ...
    this.on(STAGE_PARTICIPANT_STREAMS_ADDED, (participantInfo, streams) => {
      ...
      if (!participantInfo.isLocal) {
        ...
        // Add audio input to Nova integration for processing
        // This allows Nova to hear all other participants in multi-party conversations
        novaS2SIntegration
    ...
    this.on(STAGE_PARTICIPANT_STREAMS_REMOVED, (participantInfo, _streams) => {
      ...
      if (!participantInfo.isLocal) {
        ...
        // Remove audio input from Nova integration
        novaS2SIntegration.removeAudioInput(participantInfo.id);
  ...
  private setupSEITranscriptSender(): void {
    ...
    // Register the transcript sender with Nova integration
    novaS2SIntegration.setSEITranscriptSender(transcriptSender);
    console.info(
      '[SEI Transcript] SEI transcript sender configured for Nova integration'
    );
  }
}
