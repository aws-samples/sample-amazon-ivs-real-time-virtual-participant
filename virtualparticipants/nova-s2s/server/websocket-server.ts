import { AppSyncSubscriber } from '@common/server/appsync-subscriber';
import { DynamoDBVpClient, VpStatus } from '@common/server/dynamodb-client';
import { downloadFileFromS3 } from '@common/server/s3';
import { VirtualParticipantLambdaClient } from '@common/server/token/lambda-token-provider';
...
import { NovaS2SProxy } from './nova-s2s-proxy';
...
export class VirtualParticipantWebSocketServer {
  ...
  private novaS2SProxy: NovaS2SProxy | null = null;
  ...
  constructor(port = 3001) {
    ...
    // Note: Nova S2S proxy is now initialized when VP joins a stage
    // to avoid maintaining sessions during idle periods
  ...
  private initializeAppSync(): void {
    if (!this.appSyncSubscriber) return;
  ...
  private handleClientMessage(
    ...
    switch (message.type) {
      ...
      // Nova S2S specific messages
      case 'audio-input':
        this.handleAudioInput(message);
        break;

      case 'nova-audio':
        this.handleNovaAudio(message);
        break;

      case 'nova-start':
        this.handleNovaStart();
        break;
      ...
      async (subscriptionData: unknown) => {
        ...
        try {
          if (
            ...
            // Get stage record from DynamoDB
  ...
  private async handleVpReady(
    ...
    try {
      ...
      this.sendMessage(client.ws, {
        type: 'vp.status_updated',
        vpId,
        status: VpStatus.AVAILABLE,
        message: 'VP is ready and available for stage invitation'
      });
  ...
  private async handleVpStatusUpdate(
    ...
    try {
      ...
      this.sendMessage(client.ws, {
        ...
        message: `VP status updated to ${message.status}`
      });
  ...
  private handleAudioInput(message: unknown): void {
    try {
      if (!this.novaS2SProxy) {
        console.warn('Nova S2S proxy not initialized, cannot send audio');

        return;
      }

      const msg = message as { audio: string };
      this.novaS2SProxy.sendAudio(msg.audio);
    } catch (error) {
      console.error('Error forwarding audio to Nova S2S:', error);
    }
  }

  private handleNovaAudio(message: unknown): void {
    try {
      if (!this.novaS2SProxy) {
        console.warn('Nova S2S proxy not initialized, cannot send audio');
      ...
      const msg = message as { audio: string; vpId: string };
      this.novaS2SProxy.sendAudio(msg.audio);
    } catch (error) {
      console.error('Error sending audio to Nova S2S:', error);
    }
  }

  private async handleNovaStart(): Promise<void> {
    try {
      // Initialize Nova S2S proxy if not already initialized
      if (!this.novaS2SProxy) {
        console.info('[WebSocketServer] Initializing Nova S2S proxy...');
        this.novaS2SProxy = new NovaS2SProxy();

        // Set up callback to forward Nova S2S messages to WebSocket clients
        this.novaS2SProxy.setAudioCallback((data) => {
          this.broadcastToClients(data);
        });
      }

      await this.novaS2SProxy.startSession();
      console.info('Nova S2S session started');
    } catch (error) {
      console.error('Error starting Nova S2S session:', error);
    }
  }

  private handleNovaStop(): void {
    try {
      if (!this.novaS2SProxy) {
        console.warn('Nova S2S proxy not initialized, cannot stop session');

        return;
      }

      this.novaS2SProxy.stopSession();
      console.info('Nova S2S session stopped');
    } catch (error) {
      console.error('Error stopping Nova S2S session:', error);
    }
  }
